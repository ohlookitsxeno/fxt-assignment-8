...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.scatter(X[y == 0][:, 0], X[y == 0][:, 1], color='#42BAED', label='Class 0')
        plt.scatter(X[y == 1][:, 0], X[y == 1][:, 1], color='#ED5443', label='Class 1')
        ...
        plt.xlabel('x1')
        plt.ylabel('x2')
        plt.title(f'Shift Distance: {shift_distances[i-1]:.2f}')
        plt.legend()
        plt.grid()
        ...
        alphas = [0.1, 0.2, 0.3]  # Increasing opacity for higher confidence levels
        for level, alpha in zip(contour_levels, alphas):
            class_1_contour = plt.contourf(xx, yy, Z, levels=[level, 1.0], colors=['#ED5443'], alpha=alpha)  # Fading red for Class 1
            class_0_contour = plt.contourf(xx, yy, Z, levels=[0.0, 1 - level], colors=['#42BAED'], alpha=alpha)  # Fading blue for Class 0
            if level == 0.7:
    ...
    plt.subplot(3, 3, 1)
    plt.plot(shift_distances, beta0_list, label='Beta0', color='#ED5443')
    ...
    plt.subplot(3, 3, 2)
    plt.plot(shift_distances, beta1_list, label='Beta1', color='#FC6B3A')
    ...
    plt.subplot(3, 3, 3)
    plt.plot(shift_distances, beta2_list, label='Beta2', color='#42BAED')
    ...
    plt.subplot(3, 3, 4)
    plt.plot(shift_distances, slope_list, label='Slope', color='#ED5443')
    ...
    plt.subplot(3, 3, 5)
    plt.plot(shift_distances, intercept_list, label='Intercept', color='#FC6B3A')
    ...
    # Plot logistic loss
    plt.subplot(3, 3, 6)
    plt.plot(shift_distances, loss_list, label='Loss', color='#42BAED')
    ...
    plt.subplot(3, 3, 7)
    plt.plot(shift_distances, margin_widths, label='Margin Width', color='#ED5443')
